...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.reading-entry:visible').count(), 20);
    assert.equal(await page.locator('#benchmarks-1 .reading-environment').count(), 20);
    ...
    assert.equal(await page.locator('.reading-entry:visible').count(), 50);
    ...
    assert.equal(await page.locator('#paper-sorted .reading-entry').count(), 50);
    ...
    assert.equal(starValues.length, 50);
